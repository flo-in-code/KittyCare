...
function EditCatForm({ editingCat, closeForm, updateCat }) {
    const [catName, setCatName] = useState(editingCat.name)
    const [catBreed, setCatBreed] = useState(editingCat.breed)
    const [catColorMarkings, setCatColorMarkings] = useState(editingCat.color)
    const [catBirthday, setCatBirthday] = useState(editingCat.birthday)
    const [catWeight, setCatWeight] = useState(editingCat.weight)
    const [vetName, setVetName] = useState(editingCat.vetName)
    const [vetPhone, setVetPhone] = useState(editingCat.vetPhone)
    const [microchipID, setMicrochipID] = useState(editingCat.microchipID)
        ...
        <div onClick={() => closeForm()} className="fixed inset-0 z-30 bg-black/40 flex items-center justify-center">
            <form onSubmit={submitHandler} onClick={(e)=> e.stopPropagation()} className="bg-white rounded-2xl p-6 min-w-sm max-w-md space-y-4">
                ...
                <div className="flex gap-2 py-2">
                    ...
                    <button type="submit" className="flex-1 border border-stone-300 bg-emerald-600 text-white rounded-md py-1">Save Edits</button>
                </div>
            </form>
        </div>
    )
